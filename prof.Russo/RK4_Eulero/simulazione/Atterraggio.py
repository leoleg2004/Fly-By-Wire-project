...
# ─────────────────────────────────────────────────────────────────────────────
# Simulazione - ATTERRAGGIO PERFETTO
# ─────────────────────────────────────────────────────────────────────────────
DT_DEFAULT = 1.0 / 60.0
T_END = 25.0
# Partiamo in avvicinamento: velocità u=300 ft/s, discesa w=15 ft/s
IC = np.array([300.0, 0.0, 15.0,    
               0.0,   0.0, 0.0])   
...
def compute_aero(state):
    ...
    # Variabili del pilota automatico
    target_w = 0.0
    target_q = 0.0
    Fx = 0.0
    
    # FASE 1: IN VOLO (Discesa verso la pista)
    # L'aereo rallenta da 300 a 200 ft/s.
    if u > 200.0:
        Fx = -8000.0     # Freno aerodinamico (la velocità u diminuisce)
        target_w = 15.0  # Scende costantemente a 15 ft/s
        target_q = 0.0   # Muso dritto/fermo
        
    # FASE 2: TOCCO A TERRA (Il muso si abbassa)
    # L'aereo è tra 200 e 100 ft/s. Le ruote dietro hanno toccato.
    elif u > 100.0:
        Fx = -15000.0    # I freni delle ruote iniziano a frenare più forte
        target_w = 0.0   # LA DISCESA SI AZZERA: w va a 0 perché siamo sulla pista
        target_q = -0.1  # IL MUSO SI ABBASSA: q diventa negativo per far scendere la ruota davanti
        
    # FASE 3: SULLA PISTA (Stabilizzato e frenata finale)
    # L'aereo è sceso sotto i 100 ft/s e si sta per fermare.
    else:
        Fx = -20000.0    # Massima frenata per fermarsi del tutto
        target_w = 0.0   # Sempre a terra (w=0)
        target_q = 0.0   # Il muso ha toccato, ora torna orizzontale e stabile (q=0)
        
    # ── IL PILOTA AUTOMATICO (Controllori Proporzionali) ──
    # Calcola la portanza per rispettare i nostri target_w
    Fz = 500.0 * (target_w - w) - (mass * q * u)
    
    # Calcola il momento per far abbassare il muso esattamente come vogliamo
    M = 40000.0 * (target_q - q)
        
    return Fx, 0.0, Fz, 0.0, M, 0.0
