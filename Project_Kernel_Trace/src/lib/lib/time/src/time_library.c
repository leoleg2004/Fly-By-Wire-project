...
void time_add_millisecs(struct timespec *initial_time, uint64_t addition) {
  ...
  initial_time->tv_nsec += addition * 1000ULL * 1000ULL;
...
uint64_t time_to_millisecs(const struct timespec *time_stamp) {
  uint64_t ms;

  ms =
      (uint64_t)((time_stamp->tv_sec * 1000) + (time_stamp->tv_nsec / 1000000));
  return ms;
}