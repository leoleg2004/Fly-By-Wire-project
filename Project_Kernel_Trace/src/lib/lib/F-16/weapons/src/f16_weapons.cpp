...
void f16_weapons_load(int parameter) {
  // F-16 Weapons Trajectory Check
  volatile double drop_point = 0;
  for (long i = 0; i < parameter * 2 * 1000; i++) {
    drop_point += (i * 0.5) * sin(i*0.1) * cos(i*0.1);
  }
}

void *PeriodicTask_WEAPONS(void *ptr) {
  t_f16_weapons_par activity = *((t_f16_weapons_par *)ptr);
  ...
  while (1) {
    ...
    f16_weapons_load(activity.parameter);
