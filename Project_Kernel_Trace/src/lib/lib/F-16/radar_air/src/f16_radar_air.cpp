...
void f16_radar_air_load(int parameter) {
  // F-16 Air-to-Air Radar Sweep
  volatile double target_distance = 0;
  for (long i = 0; i < parameter * 2 * 1000; i++) {
    target_distance += tan(i) * cos(i) + sin(i);
  }
}

void *PeriodicTask_RADAR_AIR(void *ptr) {
  t_f16_radar_air_par activity = *((t_f16_radar_air_par *)ptr);
  ...
  while (1) {
    ...
    f16_radar_air_load(activity.parameter);
