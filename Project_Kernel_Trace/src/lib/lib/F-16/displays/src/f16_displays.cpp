...
void f16_displays_load(int parameter) {
  // F-16 HUD/MFD Vector Generation
  volatile double draw_line = 0;
  for (long i = 0; i < parameter * 2 * 1000; i++) {
    draw_line += sin(i) + cos(i) + tan(i*0.1);
  }
}

void *PeriodicTask_DISPLAYS(void *ptr) {
  t_f16_displays_par activity = *((t_f16_displays_par *)ptr);
  ...
  while (1) {
    ...
    f16_displays_load(activity.parameter);
