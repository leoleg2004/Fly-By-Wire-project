...
double state_time_in(const std::vector<Interval> &intervals, double lo, double hi) {
    double total = 0;
...
int main(int argc, char *argv[]) {
    ...
    // nuova struttura Mappa di routing per i PID Virtuali
    std::map<int, int> real_to_vpid;//il primo è il vecchio pid il secondo è il nuovo pid
    ...
    while (std::getline(file, line)) {
        ...
        if (std::regex_search(line, m, line_re)) {
            ...
            // Calcola il VPID corrente per questo PID OS reale
            int vpid = real_to_vpid.count(pid) ? real_to_vpid[pid] : pid;//vede se il pid che leggo va inversione ovvero dentro la mappa ce qualcosa cambia il pid altrimenti rimane invariato
//praticamente fin quando non trova il mid con il marker di inversione funziona normalmente seguendo i pid normalmente e vine salvato nel vettore tasks[pid] e quando trova il marker che scatta l'if viene creato il nuovo pid virtuale con cui traccio il cambio di priorità cosi dopo per le righe dopo l'inversione faranno riferimentos empre al nuovo thread dato che ora il
            //real_to_vpid.count(prev_pid) sarà pieno e quindi verrà salvaot il nuovo cassetto nel vettore tasks[newvpid] e quindi per le righe dopo che avrann ancora il vecchio pid faranno riferimento al nuovo pid per salvare gli eventi di switch_in e switch_out per le statistiche per il nuovo thread

            //quanfo il sistema operativo toglie un thread mìper mettere quello nuovo con le nuove caratteristiche registriamo uno sched_switch
            if (event == "sched_switch") {
                std::smatch sm;
                if (std::regex_search(details, sm, switch_re)) {
                    int prev_pid = std::stoi(sm[1]);//il pid vecchio che cede il posto al nuovo
                    char state = sm[2].str()[0];//perchè si alza quindi S o R
                    int next_pid = std::stoi(sm[3]);//chi si sta sedendo

                    // Mappiamo anche chi esce e chi entra ai loro rispettivi VPID
                    int v_prev = real_to_vpid.count(prev_pid) ? real_to_vpid[prev_pid] : prev_pid;
                    int v_next = real_to_vpid.count(next_pid) ? real_to_vpid[next_pid] : next_pid;
                    //ci salviamo gli switch in e switch_out per il nuovo cassetto virtuale per vedere le statistiche relative al nuovo thread
                    if (tasks.count(v_next)) tasks[v_next].switch_in.push_back(t);
                    if (tasks.count(v_prev)) tasks[v_prev].switch_out.push_back({t, state});
                }

                //serve solo per pulire i marker nel caso non sia un marker di inversione
                ...
                // ================================================================
                // logica inversione di priorità
                // ================================================================
                if (marker_text.find("INVERSION_DONE") != std::string::npos) {

                    std::cout << "\n[DEBUG] Ho intercettato il marker! Stringa grezza: '" << marker_text << "'\n";

                    // Regex rilassata per estrarre nome, priorita e periodo
                    std::regex inv_re(R"(INVERSION_DONE_([a-zA-Z0-9_]+)_PRIO_(\d+)_PER_(\d+))");
                    std::smatch sm_inv;
                    ...
                    if (std::regex_search(marker_text, sm_inv, inv_re)) {
                        ...
                        std::cout << "[***]  Creazione nuovo VPID per " << base_name
                                  << " (Nuovo Periodo: " << new_period_ms << " ms) al tempo " << t << "\n\n";

                        int old_vpid = vpid;
                        int new_vpid = old_vpid + 1000000;

                        // 1. Chiudiamo fittiziamente il blocco RUN del task "vecchio"
                        if (tasks.count(old_vpid)) {
                            tasks[old_vpid].switch_out.push_back({t, 'R'});
                        }

                        // 2. Aggiorniamo la mappa
                        real_to_vpid[pid] = new_vpid;

                        // 3. Inizializziamo il nuovo task per il terminale/CSV
                        ...
                        // 4. Apriamo fittiziamente il blocco RUN del nuovo task
                        tasks[new_vpid].switch_in.push_back(t);
                        tasks[new_vpid].markers.push_back({t, marker_text});

                    } else {
                        std::cout << "[ERRORE REGEX] Non riesco a scomporre la stringa del marker.\n";
                    }
    ...
    // =========================================================================
    // FASE 4: COSTRUZIONE INTERVALLI + ESPORTAZIONE CSV + STATISTICHE AVANZATE
    // =========================================================================
    ...
    for (auto &pair : tasks) {
        ...
        // --- Costruzione intervalli DDS_MSG ---
        ...
        std::cout << " [T.Risposta] WCRT : " << std::setw(10)
                  << (worst_slack_ms >= 1e9 ? 0.0 : ((calc_deadline_sec * 1000.0) - worst_slack_ms))
                  << " ms\n";
