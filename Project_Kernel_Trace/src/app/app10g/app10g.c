...
int main(int argc, char *argv[]) {
  ...
  struct sched_param param3, param4;
  int ret_err;
  ...
  // Set per il Thread 3 -> CORE 1
  CPU_ZERO(&cpuset_core1);
  CPU_SET(1, &cpuset_core1);
  pthread_attr_setaffinity_np(&attr3, sizeof(cpu_set_t), &cpuset_core1);

  // Set per il Thread 4 -> CORE 2
  CPU_ZERO(&cpuset_core2);
  CPU_SET(2, &cpuset_core2);
  pthread_attr_setaffinity_np(&attr4, sizeof(cpu_set_t), &cpuset_core2);

  // ==========================================
  // -- THREAD 3 -- (Core 1)
  // ==========================================
  t_activity_par activity_3;
  snprintf(activity_3.name, 15, "Activity_3");
  ...
  snprintf(activity_4.name, 15, "Activity_4");
  ...
  if (param4.sched_priority < 1) param4.sched_priority = 1;
