...
// struttura per edf del kernel lho visto online
struct sched_attr {
  __u32 size;
  __u32 sched_policy;
  __u64 sched_flags;
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "====================================================\n";
  for (unsigned int i = 0; i < NUM_THREADS; i++) {
    std::string type_name = (arg[i].type == TYPE_PUBLISHER) ? "PUB " : "SUB ";
    std::cout << "[" << type_name
...
void *Task(void *ptr) {
  ...
  for (int i = 0; i < iterazioni_da_fare; i++) {
    ...
    if (arg->type == TYPE_PUBLISHER) {
      ...
      while (arg->reader->take_next_sample((void *)&stats, &info) ==
             RETCODE_OK) {
        if (info.valid_data) {
    ...
    std::cout << "[" << type << "] Alt:" << std::setw(5)
              << (int)simulated_altitude << " | " << std::left << std::setw(20)
