...
void *PeriodicTask(void *ptr) {
  ...
  while (1) {
    ...
    char marker_end[128];
    snprintf(marker_end, sizeof(marker_end), "--- COMPUTATION_END: %s ---\n",
             activity.name);
    write_trace_marker(marker_end);
    ...
    if (skip) {
      ...
      if (exec_end_time > exec_next_release_time) {
        printf("%s:   -MISS-   cost                    = %ld millisecs\n",
               activity.name, computational_cost);
        char marker_miss[128];
        snprintf(marker_miss, sizeof(marker_miss),
                 "!!! DEADLINE_MISS: %s !!!\n", activity.name);
        write_trace_marker(marker_miss);
        skip = true;
      } else
...
int main(int argc, char *argv[]) {
  ...
  // Imposto la policy Real-Time FIFO (necessaria per Rate Monotonic)su entrambi
  // i thread
  pthread_attr_setschedpolicy(&attr1, SCHED_FIFO);
  pthread_attr_setschedpolicy(&attr2, SCHED_FIFO);

  // Configuro il CPU set per bloccare i thread sul CORE 0
  CPU_ZERO(&cpuset);
  CPU_SET(0, &cpuset);
  ...
  // Passo attr1 invece di NULL qua modifico gli attributi quindi non vengono
  // messi quelli di default ma quelli modificati per la schedulazione RM
  ...
  // Passo attr2 invece di NULL qua modifico gli attributi quindi non vengono
  // messi quelli di default ma quelli modificati per la schedulazione RM
  ret_err = pthread_create(&thread1, &attr1, PeriodicTask, (void *)&activity_1);
